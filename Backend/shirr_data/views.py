...
def api_file_upload_view(request):
    ...
    try:
        ...
        if df_parsed is not None and not df_parsed.empty:
            ...
            existing_data = request.session.get('sales_data', [])
            request.session['sales_data'] = existing_data + new_data
            request.session.modified = True  
            response = {
                'message': f'Successfully uploaded {len(new_data)} records.',
                'total_records': len(request.session['sales_data']),
            }
        else:
            response = {'message': 'No valid data parsed from the file.'}
